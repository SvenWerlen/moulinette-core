...
export class Moulinette {
  ...
  static async getOrCreateFolder(publisher, pack, folderType) {
    ...
    if( moulinetteFolder.length == 0 ) {
      moulinetteFolder = await Folder.create({name:"Moulinette", type: folderType})
    } else {
    ...
    if( publisherFolder.length == 0 ) {
      if(game.version.startsWith("12.")) {
        publisherFolder = await Folder.create({name: publisher, type: folderType, folder: moulinetteFolder })
      } else {
    ...
    if( packFolder.length == 0 ) {
      if(game.version.startsWith("12.")) {
        packFolder = await Folder.create({name: pack, type: folderType, folder: publisherFolder })
      } else {
